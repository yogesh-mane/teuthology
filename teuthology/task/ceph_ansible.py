...
class CephAnsible(Task):
    ...
    def setup(self):
        ...
        if self.playbook is not None:
            ...
            pb_buffer.seek(0)
            with NamedTemporaryFile(prefix='ceph_ansible_playbook_', dir='/tmp/',
                                    delete=False) as playbook_file:
        ...
        gvar = yaml.safe_dump(extra_vars, default_flow_style=False)
    ...
    def get_osd_disk_map(self, ctx, remote):
        ...
        if proc.stdout is not None:
            ...
            log.info("No output from ls {}".format(osddir))
            assert False
    ...
    def _write_hosts_file(self, prefix, content):
        ...
        with NamedTemporaryFile(prefix=prefix, delete=False) as hosts_file:
        ...
        if ctx.archive is not None and \
            ...
            for remote in self.each_cluster.only(wanted).remotes.keys():
                ...
                if ctx.config.get('coverage', False):
                    coverage_dir = os.path.join(sub, 'coverage')
    ...
    def run_rh_playbook(self):
        ...
        rhbuld = self.config.get('rhbuild')
        # skip cdn's for rhel beta tests which will use GA builds from Repo
        if self.ctx.config.get('redhat').get('skip-subscription-manager', False) is False:
            if rhbuld in GA_BUILDS:
